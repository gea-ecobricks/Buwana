/*-----------------------------------
TEXT TRANSLATION SNIPPETS FOR signup-4.php
-----------------------------------*/
const id_Page_Translations = {

    "001-email-confirmed": "✔ Email Anda telah dikonfirmasi!",
    "002-now": "Sekarang",
    "003-now-localize-you": " mari kita sesuaikan lokasi Anda.",
    "004-lets-determine-bioregion": "Mari kita tentukan bioregion dan komunitas lokal Anda.",
    "005-your-neighbourhood": "Lingkungan Anda...",
    "006-start-typing-neighbourhood": "Mulailah mengetik nama lingkungan Anda, dan <a href=\"#\" onclick=\"openAboutOSM()\" class=\"underline-link\">openstreetmaps.org</a> akan melengkapi sisanya.",
    "007-in-which-river-basin": "Di daerah aliran sungai mana Anda tinggal?",
    "008-the map shows": " ℹ️ Peta menunjukkan sungai dan aliran air di sekitar Anda. Pilih yang menjadi aliran air dari tempat Anda.",
    "010-select-your-river": "👉 Pilih sungai lokal Anda...",

    "011-watershed-select": "Ke daerah aliran sungai (DAS) mana air lokal Anda mengalir?",
    "011c-unknown": "Saya tidak tahu",
    "011d-unseen": "Saya tidak melihat sungai/aliran lokal saya",
    "011e-no-watershed": "Tidak ada daerah aliran sungai",
    "011f-fetch-error": "Terjadi kesalahan saat mengambil data sungai",
    "011b-no-rivers-found": "Tidak ditemukan sungai di dekat Anda. :-(",

    "011-non-political": "Ya! Kami menggunakan <a href=\"#\" onclick=\"openAboutRiverBasins()\" class=\"underline-link\">bioregion daerah aliran sungai</a> sebagai cara alternatif, netral dan berbasis ekologi untuk melokalkan pengguna kami.",

    "012-next-button": "Berikutnya ➡",

    "013-watershed-title": "Daerah Aliran Sungai",
    "014-watershed-description": "Daerah aliran sungai (DAS) adalah wilayah yang ditentukan oleh aliran air hujan, salju mencair, atau es yang mengalir menuju satu titik, biasanya sungai, danau, atau laut. DAS membentuk bioregion alami yang biasanya dibatasi oleh punggung bukit atau pegunungan. DAS memainkan peran ekologis yang sangat penting dan menyediakan air untuk kebutuhan manusia.",

    "015-osm-description": "Kami dengan sengaja tidak menggunakan Google Maps dan sebagai gantinya menggunakan OpenStreetMap (sebuah yayasan, bukan perusahaan berorientasi keuntungan) untuk menentukan lokasi pengguna. OpenStreetMap dibangun oleh komunitas pemeta yang berkontribusi dan memelihara data tentang jalan, sungai, jalur, dan banyak lagi di seluruh dunia. OpenStreetMap adalah data terbuka dan sumber terbuka: siapa pun bebas menggunakannya untuk tujuan apa pun selama memberikan kredit kepada OpenStreetMap dan para kontributornya.",

    "010-are-you-part-community": "Apakah Anda bagian dari komunitas lokal di Buwana?",
    "011-community-connect": "Pilih komunitas Anda...",
    "012-start-typing-community": "Buwana membantu komunitas terhubung. Mulailah mengetik untuk mencari berdasarkan nama komunitas. Seseorang di dekat Anda mungkin sudah menambahkan grup Anda!",

};
