...
LOGIN BETA (June) — based on js/login.js

This is an isolated beta of the login behaviour used by en/login-beta-june.php.
It is functionally identical to js/login.js with two deliberate fixes:

  (1) Duplicate-ID fix: the password/code TOGGLE radios are referenced by their
      new unique ids (#toggle-password / #toggle-code) instead of #password /
      #code, which previously collided with the password text input and the
      code form. The auto-code path's #code-toggle reference is corrected too.

  (2) Cleanup: updateFormVisibility() / updateButtonVisibility() /
      updateFormAction() and shakeElement() are defined ONCE at module scope so
      every call site (toggle clicks AND the auto-code path) reaches a single,
      correct implementation. The previous file defined some of these twice in
      separate closures and others not at all in the scope that used them.

The live js/login.js is intentionally left untouched.

---------------------------------------- */


/* Shared helper — single definition reachable from every closure below. */
...
/* Toggle the visible form (password vs code) based on the checked radio. */
function updateFormVisibility() {
    ...
    } else if (codeToggle && codeToggle.checked) {
...
/* Toggle which submit/send button is visible based on the checked radio. */
function updateButtonVisibility() {
    ...
    if (!submitPasswordButton || !sendCodeButton) return;
...
/* Switch the form action + required attribute based on the checked radio. */
function updateFormAction() {
    ...
    } else if (passwordToggle && passwordToggle.checked) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Event listener for toggle button clicks — uses the shared module-scope
    // updateFormVisibility / updateButtonVisibility / updateFormAction helpers.
    document.querySelectorAll('.toggle-button').forEach(button => {
        button.addEventListener('click', () => {
            if (button.classList.contains('password')) {
                ...
                if (codeToggle) codeToggle.checked = true;
